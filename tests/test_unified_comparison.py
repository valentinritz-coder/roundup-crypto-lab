...
SCHEDULE = [
    ...
    {"contributed_at": "2026-02-01T00:00:00+00:00", "amount": "40", "kind": "monthly"},
]


def metric_block(final_value: str = "80", twr: str = "0") -> dict[str, object]:
    return {
        ...
        "simple_return_on_contributions": str((float(final_value) - 80) / 80),
        ...
        "active_investor_cash_flow_simulation": [
            {
                ...
                "contribution_schedule": schedule or copy.deepcopy(SCHEDULE),
            }
        ],
    }
...
def test_incompatible_inputs_cannot_be_ranked_as_one_scenario(
    active: dict[str, object], passive: dict[str, object]
) -> None:
    with pytest.raises(ValueError, match="do not share one comparable scenario"):
        build_unified_comparison([active], [passive], strict_single_scenario=True)


def test_btc_and_eth_remain_separate_without_strict_mode() -> None:
    result = build_unified_comparison(
        [active_payload(pair="BTC/EUR"), active_payload(pair="ETH/EUR")],
        [passive_payload(pair="BTC/EUR"), passive_payload(pair="ETH/EUR")],
    )
    assert len(result["scenario_groups"]) == 2
    assert {group["scenario"]["pair"] for group in result["scenario_groups"]} == {
        "BTC/EUR",
        "ETH/EUR",
    }
    assert "not a diversified portfolio" in result["interpretations"]["pair_scope"]
...
def test_duplicate_method_identifier_fails() -> None:
    ...
    with pytest.raises(ValueError, match="duplicate scenario and method identifier"):
        build_unified_comparison([active_payload()], [duplicate], strict_single_scenario=True)
...
def test_ordering_is_deterministic() -> None:
    ...
    methods = [row["method"] for row in result["scenario_groups"][0]["rows"]]
    assert methods == sorted(methods[:2]) + ["MonthlyDCA"]


def test_nonfinite_boolean_and_invalid_drawdown_are_rejected() -> None:
    nan_payload = active_payload()
    nan_payload["active_investor_cash_flow_simulation"][0]["cash_flow_metrics"][
        "final_value"
    ] = float("nan")
    ...
    boolean_payload["active_investor_cash_flow_simulation"][0]["cash_flow_metrics"][
        "total_fees"
    ] = True
    ...
    drawdown_payload["active_investor_cash_flow_simulation"][0]["cash_flow_metrics"][
        "max_drawdown_time_weighted"
    ] = "1.1"
    with pytest.raises(ValueError, match="between zero and one"):
        build_unified_comparison([drawdown_payload], [passive_payload()])


def test_json_output_contains_no_nan_or_infinity() -> None:
    result = build_unified_comparison([active_payload()], [passive_payload()])
...
def test_passive_event_modes_use_initial_only_for_one_shot() -> None:
    ...
    one_shot = _events_for_mode(plan, start, end, "one_shot_capital")
    recurring = _events_for_mode(plan, start, end, "recurring_monthly_contributions")
