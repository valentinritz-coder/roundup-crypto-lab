...
Every passive method receives one shared investment plan: initial capital and the same monthly budget are credited as investor cash at the configured dates. Deployment timing (immediate, daily, or weekly) is separate from those cash flows, so `portfolio_value` includes both uninvested cash and crypto; DCA drawdown uses a contribution-neutral time-weighted equity curve. Run **Actions → Passive benchmarks** after
